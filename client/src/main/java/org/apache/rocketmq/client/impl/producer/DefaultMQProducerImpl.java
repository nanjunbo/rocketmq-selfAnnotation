...
public class DefaultMQProducerImpl implements MQProducerInner {
    ...
    private void checkConfig() throws MQClientException {
        //校验producer的producerGroup是否合法。
        Validators.checkGroup(this.defaultMQProducer.getProducerGroup());
        //这里有的问题，上面已经判断过了，这里还判断，没什么必要
        if (null == this.defaultMQProducer.getProducerGroup()) {
            throw new MQClientException("producerGroup is null", null);
        }
        //不能起名字叫默认的DEFAULT_PRODUCER
    ...
    private TopicPublishInfo tryToFindTopicPublishInfo(final String topic) {
        ...
        if (topicPublishInfo.isHaveTopicRouterInfo() || topicPublishInfo.ok()) {
            ...
            //再从nameServer获取一下topic信息，这时候传入defaultMQProducer，创建的topic为AUTO_CREATE_TOPIC_KEY
    ...
    private SendResult sendKernelImpl(final Message msg,
        ...
        if (brokerAddr != null) {
            ...
            try {
                ...
                //发送信息校验一下是否禁用。但是看代码里面往checkForbiddenHookList里塞数据的引用，这个应该是废弃了
                ...
                //发送消息的前置逻辑,判断如果是事务消息，如果是事务消息，就设置事务消息的
